...
def _normalize_taxon_history(obj, path=()):
    ...
    if isinstance(obj, dict):
        ...
        for k, v in obj.items():
            ...
            ren = {
                ...
                # "prevLineageNames" : "previousLineage",
    ...
    if isinstance(obj, list):
        # If we are inside the top-level "taxa" array
        # if path and path[-1] == "taxa":
            # Sort deterministically so PHP & C# align.
            #  - The C# service happens to come out sorted by taxnodeID ASC
            #  - The PHP service is usually reversed, so we just sort.
            # obj = sorted(obj, key=lambda d: d.get("taxnodeID", 0))
            # test
            # print("reversing order of taxa")
